...
def create_challenge(db: Session, username:str, challenge: str):
	db_challenge = db.query(_models.Challenge).filter(_models.Challenge.username == username).first()
	db_challenge.challenge = challenge
	db.commit()
	challenge = _schemas.Challenge(pass_username=db_challenge.pass_username, deviceidhash=db_challenge.deviceidhash, challenge=db_challenge.challenge)
	return challenge
